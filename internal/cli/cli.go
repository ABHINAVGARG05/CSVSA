	...
	func (c *CLI) runScan(cmd *cobra.Command, args []string) error {
		...
		if err := enricher.EnrichConsensusResult(ctx, consensusResult); err != nil {
			fmt.Fprintf(os.Stderr, "Warning: KEV enrichment failed: %v\n", err)
		} else if c.config.Verbose {
			fmt.Printf("KEV catalog loaded (%d known exploited vulns checked)\n", kevClient.Count())
			if consensusResult.Statistics.KEVCount > 0 {
				fmt.Printf("%d vulnerabilities are actively exploited (CISA KEV)\n",
					consensusResult.Statistics.KEVCount)
			}
		}
